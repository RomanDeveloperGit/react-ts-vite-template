...
Обратиться к переменным окружения можно через import.meta.env.*, где * - имя переменной из .env.

**Имена переменных должны начинаться с приставки APP. Примеры: APP_BACKEND_HOST, APP_BACKEND_PORT и т.д.**

**Чтобы типизировать .env, в файле env.d.ts небходимо добавить новые свойства для интерфейса ImportMetaEnv.**
